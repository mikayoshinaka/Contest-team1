...
public class fellowshipSpawn : MonoBehaviour
{
    ...
    //リスポーン時使用変数
    bool follow1, follow2, follow3, follow4, limit, start = false, repeat = false, Jd1, Jd2;
    public static bool startSpawn;
    public static bool judgeCircle1, judgeCircle2;
    ...
    void Update()
    {
        ...
        //Debug.Log("circle1" + width1);
        //Debug.Log("circle2" + width2);
        //円の判定
        ...
        if (circle1.activeSelf && circle2.activeSelf)
            if (C1judge1 <= this.transform.position.x && C1judge2 >= this.transform.position.x &&
                C1judge3 <= this.transform.position.z && C1judge4 >= this.transform.position.z && SurroundScript.END && !repeat)
            {
                repeat = true;
                judgeCircle1 = true;
                Jd1 = true;
        ...
        if (circle1.activeSelf && circle2.activeSelf)
            if (C2judge1 <= this.transform.position.x && C2judge2 >= this.transform.position.x &&
                C2judge3 <= this.transform.position.z && C2judge4 >= this.transform.position.z && SurroundScript.END && !repeat)
            {
                repeat = true;
                judgeCircle2 = true;
                Jd2 = true;
        ...
        if (judgeCircle1 && judgeCircle2 && (Jd1 || Jd2))
            start = true;
